...
contract OTCSwap is ReentrancyGuard, Ownable {
    ...
    function fillOrder(uint256 orderId) external nonReentrant validOrder(orderId) {
        ...
        // First transfer: buyToken from taker to maker
        try this.attemptTransfer(IERC20(order.buyToken), order.maker, order.buyAmount) {
            // Second transfer: sellToken from contract to taker
            try this.attemptTransfer(IERC20(order.sellToken), msg.sender, order.sellAmount) {
                emit OrderFilled(
                    orderId,
                    order.maker,
                    msg.sender,
