...
describe('OTCSwap', function () {
  ...
  describe('Order Creation', function () {
    ...
    it('should create a public order and track it in active orders', async function () {
      ...
      const [makers, , sellTokens, sellAmounts, , , , actives] = await otcSwap.getActiveOrders(0, 1)
    ...
    it('should emit OrderCreated event with correct timestamp', async function () {
      ...
      expect(events[0].args.createdAt).to.be.closeTo(BigInt(currentTime), BigInt(2))
    })
  })
  ...
  describe('Active Orders Management', function () {
    ...
    beforeEach(async function () {
      ...
      // Create orders alternating between alice and bob
    ...
    it('should maintain active orders correctly after operations', async function () {
      ...
      const [finalMakers, , , , , , , actives] = await otcSwap.getActiveOrders(0, 5)
      ...
      const allOrders = await Promise.all(
        Array(5).fill().map((_, i) => otcSwap.orders(i))
      )
    ...
    it('should handle pagination correctly', async function () {
      // Get first page
      const [makers1, , , , , , , , nextOffset1] = await otcSwap.getActiveOrders(0, 2)
      expect(makers1.length).to.equal(2)

      // Get second page
      const [makers2, , , , , , , , nextOffset2] = await otcSwap.getActiveOrders(nextOffset1, 2)
      expect(makers2.length).to.equal(2)

      // Since we alternated between alice and bob when creating orders,
      // consecutive orders should have different makers
      expect(makers1[0]).to.not.equal(makers1[1])
      expect(makers2[0]).to.not.equal(makers2[1])
    })
  ...
  describe('Order Expiry', function () {
    ...
    it('should handle orders near expiry boundary', async function () {
      ...
      await expect(otcSwap.connect(bob).fillOrder(0)).to.be.revertedWith('Order expired')
  ...
  describe('Gas Optimization Tests', function () {
    ...
    it('should show consistent gas costs for order lifecycle', async function () {
      ...
      for (let i = 1; i < gasCosts.length; i++) {
        const percentChange = (Number(gasCosts[i].gasUsed - gasCosts[i - 1].gasUsed) / Number(gasCosts[i - 1].gasUsed) * 100).toFixed(2)
      ...
      for (let i = 1; i < fillGasCosts.length; i++) {
        const percentChange = (Number(fillGasCosts[i].gasUsed - fillGasCosts[i - 1].gasUsed) / Number(fillGasCosts[i - 1].gasUsed) * 100).toFixed(2)
      ...
      gasCosts.forEach(({ gasUsed }, index) => {
        if (index > 0) {
          ...
          expect(difference).to.be.lessThan(50000,
    ...
    it('should analyze gas costs for different operations', async function () {
      ...
      console.log('\nRelative gas costs:')
      console.log(`Fill is ${(Number(fillGas) / Number(createGas) * 100).toFixed(1)}% of creation cost`)
      console.log(`Cancel is ${(Number(cancelGas) / Number(createGas) * 100).toFixed(1)}% of creation cost`)
    })
  })
})
